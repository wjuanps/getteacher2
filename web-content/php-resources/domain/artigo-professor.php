...
session_start();
    ...
    $id_post = (int) utf8_decode(strip_tags(trim(filter_input(INPUT_GET, 'id_post', FILTER_SANITIZE_NUMBER_INT))));
    $id_professor = utf8_decode(strip_tags(trim(filter_input(INPUT_GET, 'id_professor', FILTER_SANITIZE_NUMBER_INT))));
    $admin = strip_tags(trim(filter_input(INPUT_GET, 'a', FILTER_SANITIZE_STRING)));
    
    if (!empty($id_post)) {
        if (!empty($admin)) {
            $getPosts = select("blog AS blog INNER JOIN professor p", "*", "WHERE blog.id_professor = p.id_professor AND blog.id_blog = '$id_post'", "ORDER BY blog.id_blog DESC");
        } else {
            $getPosts = select("blog AS blog INNER JOIN professor p", "*", "WHERE blog.id_professor = p.id_professor AND blog.id_blog = '$id_post' AND status = 1", "ORDER BY blog.id_blog DESC");
        }
    } else {
        header("Location: blog-dos-professores.php");       
    }
                    ...
                    <div class="divPanel notop page-content">
                        ...
                        <div class="row-fluid">
                            <div class="breadcrumbs span12">
                                <a href="../../../">Home</a> &nbsp;/&nbsp; <span class="username"><a href="artigos-do-professor.php?professor=<?php echo $id_professor; ?>">Blog <?php echo ($genero != '') ? ($genero == "Masculino") ? "do professor " : "da professora " : ""; echo $nome; ?></a> &nbsp;/&nbsp; Post professor </span>
                                        ...
                                        <div class="span8">
                                            <div class="box box-widget">
                                                ...
                                                <div style="text-align: left;" class='box-body'>
                                                    ...
                                                    <a href=""><img class="img-responsive" style="width: 800px; height: 300px;" src="../area-restrita/portifolio/<?php echo $post->imagem; ?>" alt="Photo"></a>
                                                    <h4><a href="" class="titulo"><strong><?php echo $post->titulo; ?></strong></a></h4>
                                                    <div style="border-radius: 5px; background-color: rgba(0,0,0,.04); padding: 15px;">
                                                        <p style="font-family: sans-serif; text-align: justify; white-space: pre-line;">
                                                            <?php echo utf8_encode($post->texto) ?>
                                                        </p>
                                                    </div>
                                                    ...
                                                    <div style="margin-top: 1%;">
                                                        ...
                                                        <?php $total_comentario = $pdo->query("SELECT * FROM comentario_blog WHERE id_blog = $post->id_blog"); ?>
                                                        <?php $total_likes = $pdo->query("SELECT * FROM like_blog WHERE id_post = $post->id_blog"); ?>
                                                        <span class='pull-right text-muted'><span class="likes_<?php echo $post->id_blog; ?>">
                                                            <?php echo $total_likes->rowCount(); ?></span> likes - <span class="coment_<?php echo $post->id_blog; ?>">
                                                            <?php echo $total_comentario->rowCount(); ?></span> comentários
                                                        </span>
